...
## [3.3.5] - 2026-05-22
### Añadido
- **Detección de Ventanas Linux con xcap y Fallback xprop**: Reemplazada la consulta exclusiva del proceso externo `xprop` por la integración directa de la librería pura de Rust `xcap v0.9.4`, que soporta de forma nativa e interactiva tanto sesiones X11 como sesiones Wayland. Se diseñó un flujo de resolución robusto de doble vía: (1) Intenta obtener el ID de ventana activa en X11 y busca sus metadatos nativos en `xcap`; (2) En ausencia del ID de ventana activa (por ejemplo, bajo Wayland puro), filtra y detecta las ventanas activas en pantalla usando heurísticas basadas en ventanas visibles no minimizadas; (3) Fallback transparente a `xprop` si la librería nativa no encuentra los metadatos.
- **Launcher e Inicialización Automática de Agente de Usuario**: Añadido script wrapper `/opt/activity-monitor/bin/activity-monitor-agent-user-launcher.sh` al instalador, el cual resuelve automáticamente las variables de entorno críticas del servidor gráfico (`DISPLAY` y `XAUTHORITY`) en la sesión interactiva del usuario. Se actualizó la persistencia `.desktop` para usar este wrapper y se configuró al instalador silencioso y estándar para iniciar el agente de usuario de manera desatendida y en tiempo real justo después de la instalación (usando `su - $USER -c "..."`) sin requerir reinicios ni logout para iniciar el rastreo de actividad.
- **Instalación Desatendida de Dependencias Gráficas**: El instalador ahora verifica y asegura la presencia de `x11-utils` en el sistema Linux para garantizar la disponibilidad absoluta de herramientas de consulta gráfica durante los fallbacks y diagnósticos.
